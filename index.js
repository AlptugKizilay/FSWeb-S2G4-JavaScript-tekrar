...
/* 	GÖREV 3:
		- Sayfanın en üstünde global değişken olarak tanımlanmış bir sayilar dizisi bulunmaktadır. Bu dizi içinde 0 ile 1000 arasında rasgele oluşturulmuş tam sayılar 
		ve ondalıklı sayılar bulunmaktadır. Bu diziyi kullanarak aşağıdakileri uygulayın:
			...
			💡 İPUCU: Tekrar edilen sayıları ve kaç kere tekrar edildiğini kaydetmek için bir nesne tanımlamalısınız, bu görevi yapabilmek için
			 en az 2 kere döngü yazmalısınız. Birinci döngüde hangi sayının kaç kere tekrar edildiğini tespit edip, 
...
/*  (oto test yok) sayilar dizisi içinde kaç adet sayı olduğunu konsola yazdırın */
	...
	let ucebolunenlerintoplami = ucetambolunenler.reduce((total , item) => {
		return total + item;
	},0);
	...
	let besyuzdenkucuksayilar = sayilar.filter(item => {
		return item < 500;
	})
	...
	let tekraredensayilarString = []; 
	let counter = 0;
	for(let i = 0; i<copySayilar.length; i++){
		let counter = 0;
		for(let j=0; j<copySayilar.length; j++){
			if(copySayilar[i]===copySayilar[j]){
				counter += 1;
			}
			
		}
		if(counter > 1) {
			//tekraredensayilarString.push(`${copySayilar[i]} sayisi ${counter} kez tekrar etmiş.`)
			tekraredensayilar.push(copySayilar[i]);
		}
	}
	...
	*/
	function findDuplicates(arr) {
		...
		for (let key in frequency) {
		  if (frequency[key] > 1) {
			duplicateNumbers.push(key + " sayısı " + frequency[key] + " kere tekrar edilmiştir.");
		  }
		}
	  ...
	  let tekraredensayilar = findDuplicates(sayilar);
	  console.log(tekraredensayilar);
